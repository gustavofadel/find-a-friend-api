...
describe('View Pet Details (e2e)', () => {
  ...
  it('should be able to view pet details', async () => {
    ...
    const viewPetDetailsResponse = await request(app.server)
      .get(`/pets/${pet.id}`)
      .send()

    expect(viewPetDetailsResponse.statusCode).toEqual(200)
    expect(viewPetDetailsResponse.body.pet).toEqual(
